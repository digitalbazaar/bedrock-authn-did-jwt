...
const STORE_NAME_SPACE = 'authn-did-jwt';
let strategy = null;
...
api.generateToken = function(did, callback) {
  if(did.indexOf('did:') !== 0) {
    return callback(new BedrockError(
      'Invalid DID specified.', 'InvalidDid',
      {'public': true, httpStatusCode: 400}));
  }
  strategy.store.sign({
    namespace: STORE_NAME_SPACE,
    payload: {
      'urn:bedrock.authn': {did: did}
    }
  }, callback);
};
...
api._verify = function(token, callback) {
  strategy.store.verify({namespace: STORE_NAME_SPACE, token: token}, callback);
};
